...
class ManualMap(cypy.Module):
	...
	def add_to_argparse(self, parser):
		arggroup = parser.add_argument_group(
			self.name, self.description, prefix_chars="-/")
		arggroup.add_argument("--clear-header", dest="clear_header", metavar="clear_header", action="store", default=1, choices=(0, 1), type=int,
                        help="clear the header of the payload after injection")
		arggroup.add_argument("--clear-unneeded-sections", dest="clear_unneeded_sections", metavar="clear_unneeded_sections", action="store", default=1, choices=(0, 1), type=int,
                        help="clear unneeded sections for the target binary to run after injection")
		arggroup.add_argument("--sehsupport", dest="sehsupport", metavar="sehsupport", action="store", default=1, choices=(0, 1), type=int,
                        help="if clearing unneeded sections, clear .pdata as well")
		arggroup.add_argument("--adjust-protections", dest="adjust_protections", metavar="adjust_protections", action="store", default=1, choices=(0, 1), type=int,
	...
	def inject(self, handler: cypy.Cypyonate, target: str, payload: str, verbose: bool):
		...
		cypy.printv("Wrote shellcode to target process")
...
if cypy.is64bit():
	SHELLCODE = \
            b"\x48\x89\x4C\x24\x08\x48\x81\xEC\x18\x01\x00\x00\x48\x83\xBC\x24\x20\x01\x00\x00\x00\x75\x15\x48\x8B" + \
	...
	SHELLCODE = \
            b"\x55\x8B\xEC\x83\xEC\x68\x83\x7D\x08\x00\x75\x0F\x8B\x45\x08\xC7\x40\x0C\x40\x40\x40\x00\xE9\xAF\x02" +\
